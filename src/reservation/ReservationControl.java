...
public class ReservationControl {
    ...
    static String userid = "50416036"; //ユーザID
    static String password = "50416036"; //パスワード
            ...
            try { // userの情報を取得するクエリ
                ...
                if (rs.next()){
                    ...
                    if ( password_from_db.equals(password)){ //認証成功:データベースのIDとパスワードに一致
                        ...
                        //認証失敗:パスワードが不一致
                        res = "ログインできまんせん.ID・パスワードがちがいます";
                    }
                } else { //認証失敗;ユーザIDがデータベースに存在しない
                    res = "ログインンできません.ID・パスワードが 違います。";
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private boolean checkReservationDate( int y, int m, int d){	//正しい日付が入力されているかの確認
        // 予約日
            	...
            	if( st.equals(et)){        //開始時刻と終了時刻が等しい
            		...
            		try {
            			...
            			while(rs.next()){
            				...
            				String start = rs.getString("start_time");
            				String end = rs.getString("end_time");
            				if (start.compareTo(st)==0 && end.compareTo(et)==0){
            					// 重複有りの場合に ng をtrueに設定
            					ng = true; break;
            				}
            			}
            			/// 重なりチェックの処理　ここまで  ///////
            			if (!ng){    //重なっていない場合
            				res = "その予約日には予約されておりません";
            			} else {    //重なっていた場合
            				int rs_int = mysql.removeReservation(rdate, st, et, reservation_userid, facility);
            				res ="キャンセルされました";
            			}
            		}catch (Exception e) {
            			e.printStackTrace();
